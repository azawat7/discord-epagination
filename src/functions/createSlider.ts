...
import { SliderOptions, ButtonNames } from "./createSlider.interfaces";

/**
 * Options for the slider.
 * @typedef SliderOptions
 *
 * @property {CommandInteraction} interaction Discord.js message resolvable.
...
export const createSlider = async (options: SliderOptions): Promise<void> => {
	...
	const { interaction, embeds, buttons, time, otherButtons } = options; // prettier-ignore
	...
	sliderMessage = await interaction.fetchReply();
	...
	async function editEmbed(interaction, index: number, state?: boolean) {
		interaction
			.update({
				embeds: [embeds[index]],
